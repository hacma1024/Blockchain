...
type StateListInterface interface {
	...
	GetStateByRange(string, string) pb.Response
...
// GetStateByRange returns logs between startTime and endTime of a Key
func (sl *StateList) GetStateByRange(startKey string, endKey string) pb.Response {
	resultsIterator, err := sl.Ctx.GetStub().GetStateByRange(startKey, endKey)
	if err != nil {
		return shim.Error(err.Error())
	}
	defer resultsIterator.Close()
	...
	buffer.WriteString("]")
	fmt.Print("Result from GetStateByRange:\n")
	fmt.Println(buffer.String())
